...
- Balance gates:
  ...
  - **As built (25 Sept 2026, decided with main: option A).** The scripted drifter failed those gates, and partly on the classes alone: in their own karts the heavy class is its fastest class on 5 of 6 tracks, and on Frostbite heavy is 3.35% under the median (G0). Kart-caused, G1 failed 138 of 480 pair-track checks, worst 4.64% (kart-controller SOP Lessons, 25 Sept: low handling charges the drift at the full rate). So the gate uses the Hard AI, the class gate's measure (balance.e2e.test.ts: the same three personalities averaged, the exact solo race, items and hazards as they are): **G0** every combo within 4% of the track's median (worst 3.18%, Frostbite, Momo in the Snack Truck; per track Boardwalk 1.52, Canyon 2.18, Frostbite 3.18, Harbor 2.03, Meadow 1.24, Skyline 1.37); **G1** every pair within 4% of the same racer in their own kart on each track (worst −3.75%, the same combo; per track Boardwalk 1.52, Canyon 1.98, Frostbite 3.75, Harbor 2.87, Meadow 1.07, Skyline 2.66) and within 1.5% over the six tracks (worst 1.16%, Gus in the Comet Pod); **G2** no combo fastest on more than 2 tracks (at most 2: Pip in the Snack Truck, same total as Momo in the Stone Stomper, on Canyon and Meadow). No claw rescue in the 702 runs. The starting numbers passed unchanged. One test per track (12 to 31 s here), 100 s in all. The scripted drifter's view is a report, not a gate: `COMBOS_REPORT=1 npx vitest run src/game/combos.report.test.ts --reporter=verbose`.
