...
- 2026-09-19: **Kart-controller finding, not fixed here**: `stepGround` snaps any `y < groundY` back to the ground before it tests `voidY`, so the controller's `respawn` event can never fire on a road sample. The race-manager respawn path is tested by injecting that event (`vi.mock` of `stepKarts` in `race.test.ts`). The real fix belongs to the kart-controller session: skip the snap when the kart is outside the road (`|lateral| > halfWidth`) on a `ground: none` track, or when the sample's ground is the void. Until then no kart can fall off Skyline Circuit.
