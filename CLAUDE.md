...
## Current state
Built and green: kart-controller, track-builder (Harbour Loop only), race-manager, ai-driver, items, ui-hud. `npm run dev` runs the game **Rascal Rally!**: title screen over an all-AI attract race, mode select (Quick, Grand Prix, Knockout, Time Trial, Daily), roster, cup select, the real HUD, pause, settings, credits, results, Grand Prix table and Knockout cut. Placeholder box karts and cube decor until art-pipeline. `src/game/` holds the loop, the chase camera and `session.ts` (one race, built and disposed per race); `src/main.ts` is the game controller. In dev, `kart` on the browser console exposes the live session, UI, renderer and `kart.stats()`. Build order from docs/build-ritual.md: kart-controller → track-builder → race-manager → ai-driver → items → ui-hud → audio → art-pipeline → vfx-juice → backend-leaderboard → performance → deploy. One system per session; each lives in `src/<system>` with headless tests.
