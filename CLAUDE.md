...
The live site is GitHub Pages (pushed builds publish themselves, but only when the Deploy run is green: after every push, check `gh run list` and the live bundle before calling anything live; on 25 Sept 2026 one failing CI test held back ten pushes for four hours while the score checker already expected the new game). Performance: Auto governor plus measured budgets (docs/sops/performance.md). Not done: a browser fps run and Lighthouse in verify (by choice, see docs/sops/performance.md); Vercel (only with the user's OK in chat). Red-team: done 24 Sept 2026 (client clean; the leaderboard fixes are live: origin allowlist, name filter, names cap, signups off, keep-alive job). Build order from docs/build-ritual.md: kart-controller → track-builder → race-manager → ai-driver → items → ui-hud → audio → art-pipeline → vfx-juice → backend-leaderboard → performance → deploy. Each system lives in `src/<system>` with headless tests.
