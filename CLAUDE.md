...
The live site is GitHub Pages (pushed builds publish themselves). Performance: Auto governor plus measured budgets (docs/sops/performance.md). Not done: the headless perf, bundle and a11y stages in verify; Vercel (only with the user's OK in chat); a final red-team of the whole client. Build order from docs/build-ritual.md: kart-controller → track-builder → race-manager → ai-driver → items → ui-hud → audio → art-pipeline → vfx-juice → backend-leaderboard → performance → deploy. Each system lives in `src/<system>` with headless tests.
