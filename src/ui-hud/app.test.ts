...
describe('app flow', () => {
  it('SOP gate: every screen and overlay is reachable by the six actions alone', () => {
    ...
    const all: string[] = ['title', 'modeSelect', 'rosterSelect', 'cupSelect', 'racing', 'results', 'gpTable', 'knockoutCut', 'pause', 'settings', 'credits'];
  ...
  it('quick race skips the cup screen and ends back at mode select', () => {
    const { trail } = walk([
      { type: 'boot' }, { type: 'start' }, { type: 'pickMode', mode: 'quick' }, { type: 'pickRacer', racerId: 'gus' },
      { type: 'raceFinished', seriesHasNext: false }, { type: 'continue' },
    ]);
    expect(trail).toEqual(['title', 'modeSelect', 'rosterSelect', 'racing', 'results', 'modeSelect']);
  });

  it('pause stacks settings, resume clears the stack, quit leaves the race', () => {
    let s = walk([{ type: 'boot' }, { type: 'start' }, { type: 'pickMode', mode: 'quick' }, { type: 'pickRacer', racerId: 'pip' }]).s;
