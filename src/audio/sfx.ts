...
export const PATCHES: Readonly<Record<SfxId, Patch>> = Object.freeze({
  ...
  boing: P({ wave: 'sine', f0: 180, f1: 720, glide: 0.35, attack: 0.003, decay: 0.45, gain: 0.45, vibrato: [60, 18] }),
  slam: P({ wave: 'sine', f0: 140, f1: 45, glide: 0.25, attack: 0.002, decay: 0.45, gain: 0.6 }),
  anchor: P({ wave: 'square', f0: 330, f1: 220, glide: 0.2, attack: 0.002, decay: 0.3, gain: 0.3, vibrato: [90, 30], filter: { type: 'bandpass', f0: 1800, q: 2 } }),
  slingshot: P({ wave: 'sawtooth', f0: 220, f1: 1100, glide: 0.18, attack: 0.002, decay: 0.3, gain: 0.3, filter: { type: 'lowpass', f0: 3000 } }),
  mouse: P({ wave: 'square', f0: 2000, attack: 0.001, decay: 0.03, gain: 0.15, steps: [0, 0, 0, 0, 12], stepDur: 0.06 }),
  ...
  ventWarn: P({ wave: 'triangle', f0: 90, f1: 160, glide: 0.8, attack: 0.05, hold: 0.5, decay: 0.3, gain: 0.3, vibrato: [14, 20], filter: { type: 'lowpass', f0: 700 } }),
  geyser: P({ wave: 'noise', f0: 400, f1: 1600, glide: 0.4, attack: 0.02, hold: 0.6, decay: 0.7, gain: 0.35, filter: { type: 'bandpass', f0: 1200 } }),
  steamVent: P({ wave: 'noise', f0: 2000, f1: 3200, glide: 0.5, attack: 0.02, hold: 0.7, decay: 0.6, gain: 0.3, filter: { type: 'highpass', f0: 1800 } }),
  loop: P({ wave: 'sawtooth', f0: 220, f1: 880, glide: 1.2, attack: 0.05, hold: 0.6, decay: 0.6, gain: 0.3, vibrato: [6, 12], filter: { type: 'lowpass', f0: 2400 } }),
  hit: P({ wave: 'square', f0: 220, f1: 110, glide: 0.2, attack: 0.002, decay: 0.25, gain: 0.4, filter: { type: 'lowpass', f0: 1200 } }),
  // your item landed on a rival: a thwack and a rising two-note chime, heard however far away
  hitConfirm: P({ wave: 'triangle', f0: 784, attack: 0.002, decay: 0.3, gain: 0.4, steps: [0, 4, 12], stepDur: 0.06 }),
  ...
  slipstream: P({ wave: 'noise', f0: 1, attack: 0.08, hold: 0.2, decay: 0.5, gain: 0.4, filter: { type: 'bandpass', f0: 500, f1: 3000, q: 0.9 } }),
  // the drift spark tiers (blue, orange, purple) climb in pitch, notes and length
