...
export function readLine(s: KartState, track: Track, m: AiMemory, sc: Scratch, out: LineInfo): LineInfo {
  ...
  const turnShort = wrapAngle(headingOf(sc.short.tangent) - h);
  out.kappa = Math.max(Math.abs(turnShort) / l.lookAheadMin, Math.abs(out.turnNear) / out.probeNear);
...
export function lateralTarget(s: KartState, m: AiMemory, line: LineInfo, seconds: number): number {
  const l = AI.line;
  ...
  const lat = clamp(lane + inside + wander, -max, max);
  // declined the branch ahead: keep to the far side so the controller does not switch us onto it
  if (m.branchChoice < 0 && line.branchAhead === -m.branchChoice && line.branchSide !== 0) {
    return -line.branchSide * Math.max(Math.abs(lat), l.declineFraction * hw);
  }
  return lat;
}
...
export function chooseBranch(s: KartState, track: Track, m: AiMemory, profile: AiProfile, line: LineInfo): void {
  ...
  for (let i = 1; i < list.length; i++) {
    ...
    const take = m.skill >= profile.shortcutSkill && next(m) < m.personality.aggression;
